...
parser.add_argument('--filter_matrix', type=str, default="/data/input/Files/P-Maize/Matrix/maize-husk-1/filter_matrix,/data/input/Files/P-Maize/Matrix/maize-husk-2/filter_matrix", help='Path to matrix file list')
parser.add_argument('--splice_matrix', type=str, default="/data/input/Files/P-Maize/Matrix/maize-husk-1/splice_matrix,/data/input/Files/P-Maize/Matrix/maize-husk-2/splice_matrix", help='Path to splice file list')
parser.add_argument('--unsplice_matrix', type=str, default="/data/input/Files/P-Maize/Matrix/maize-husk-1/RNAvelocity_matrix,/data/input/Files/P-Maize/Matrix/maize-husk-2/RNAvelocity_matrix", help='Path to unsplice file list')
parser.add_argument('--sample_value', type=str, default="husk1,husk2", help='Path to sample names file')
...
filter_matrix = args.filter_matrix
splice_matrix = args.splice_matrix
unsplice_matrix = args.unsplice_matrix
...
directory_path = Path(f"./{sample_value}/filter")
directory_path.mkdir(parents=True, exist_ok=True)
folder_path_filter = os.path.abspath(directory_path)
matrixfile = filter_matrix + '/matrix.mtx.gz'
featuresfile = filter_matrix + '/features.tsv.gz'
barcodesfile = filter_matrix + '/barcodes.tsv.gz'
copy_and_process(matrixfile, featuresfile, barcodesfile, folder_path_filter)

if filter_matrix != splice_matrix and filter_matrix != unsplice_matrix and splice_matrix != unsplice_matrix:
    directory_path = Path(f"./{sample_value}/splice")
    directory_path.mkdir(parents=True, exist_ok=True)
    folder_path_splice = os.path.abspath(directory_path)
    matrixfile = splice_matrix + '/matrix.mtx.gz'
    featuresfile = splice_matrix + '/features.tsv.gz'
    barcodesfile = splice_matrix + '/barcodes.tsv.gz'
    copy_and_process(matrixfile, featuresfile, barcodesfile, folder_path_splice)

    directory_path = Path(f"./{sample_value}/unsplice")
    directory_path.mkdir(parents=True, exist_ok=True)
    folder_path_unsplice = os.path.abspath(directory_path)
    matrixfile = unsplice_matrix + '/unspliced.mtx.gz'
    featuresfile = unsplice_matrix + '/features.tsv.gz'
    barcodesfile = unsplice_matrix + '/barcodes.tsv.gz'
    copy_and_process(matrixfile, featuresfile, barcodesfile, folder_path_unsplice)
else:
    folder_path_splice = folder_path_filter
    folder_path_unsplice = folder_path_filter


adata = sc.read_10x_mtx(folder_path_filter, var_names='gene_ids')
...
if trans_matrix_list[i]==trans_splice_list[i]==trans_unsplice_list[i]:
    ...
    adata_splice = sc.read_10x_mtx(folder_path_splice, var_names='gene_ids')
    adata_unsplice = sc.read_10x_mtx(folder_path_unsplice, var_names='gene_ids')
    ...
    print(f"sample: {sample_value}, genes in matrix/splice/unsplice/union: {len(genes_filter)}/{len(genes_splice)}/{len(genes_unsplice)}/{len(all_genes)}")
    ...
    print(f"sample: {sample_value}, cells in matrix/splice/unsplice/union: {len(cells_filter)}/{len(cells_splice)}/{len(cells_unsplice)}/{len(all_cells)}")
...
adata.obs_names = [f"{cell_name}_{sample_value}" for cell_name in adata.obs_names]
print(adata.obs_names[:10])
adata.obs['biosample'] = biosample_value
adata.obs[group_key] = sample_value
